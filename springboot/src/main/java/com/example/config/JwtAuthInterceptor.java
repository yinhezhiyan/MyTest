...
public class JwtAuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (uri.startsWith("/api/admin/") && !"ADMIN".equals(role)) {
            throw new CustomException(ResultCode.FORBIDDEN, "无权限访问管理员接口");
        }
        if (uri.startsWith("/api/student/") && !"STUDENT".equals(role)) {
            throw new CustomException(ResultCode.FORBIDDEN, "无权限访问学生接口");
        }
        if ("STUDENT".equals(role) && (uri.startsWith("/api/users") || uri.startsWith("/api/exercises") || uri.startsWith("/api/knowledge-points") || uri.startsWith("/api/recommendations/logs"))) {
            throw new CustomException(ResultCode.FORBIDDEN, "无权限访问管理员接口");
        }
